...
def compare_core(h2h_consensus, tbpore_consensus, H37RV_genome):
    with open(h2h_consensus) as h2h_consensus_fh, open(
        tbpore_consensus
    ) as tbpore_consensus_fh, gzip.open(H37RV_genome, "rt") as H37RV_genome_fh:
        ...
        assert {
            len(h2h_consensus_records),
            len(tbpore_consensus_records),
            len(H37RV_genome_records),
        } == {1}
    ...
    assert (
        len({len(h2h_consensus_seq), len(tbpore_consensus_seq), len(H37RV_genome_seq)})
        == 1
    )
    ...
    for h2h_base, tbpore_base, ref_base in zip(
        h2h_consensus_seq, tbpore_consensus_seq, H37RV_genome_seq
    ):
...
def compare(h2h_consensus_glob_pattern, tbpore_glob_pattern, output):
    ...
    with open(output, "w") as output_fh:
        for sample in common_samples:
            ...
            stats_to_count = compare_core(
                h2h_consensus, tbpore_consensus, snakemake.params.H37RV_genome
            )
            print(
                f"Comparing {sample}:\n{pretty_print_stats(stats_to_count)}",
                file=output_fh,
            )
